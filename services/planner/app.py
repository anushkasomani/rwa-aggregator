...
from .schema import Plan, GOOD_THRESH, BAD_THRESH, Gates, Weighting # Import Weighting
import google.generativeai as genai
import os
import json 
from dotenv import load_dotenv
from .plan_analyzer import analyze_features
...
def map_sentiment_words(s: str) -> str:
    """Replace descriptive sentiment terms with numbers (case-insensitive)."""
    s = re.sub(r"SENTIMENT\s*>=\s*GOOD", f"SENTIMENT >= {GOOD_THRESH}", s, flags=re.IGNORECASE)
    s = re.sub(r"SENTIMENT\s*<=\s*BAD",  f"SENTIMENT <= {BAD_THRESH}",  s, flags=re.IGNORECASE)
    return s

def normalize_gate_expr(expr: str) -> str:
    """Normalize PRICE/CLOSE and SMA spellings so downstream code is happy."""
    e = (expr or "").strip()
    # PRICE → CLOSE
    e = re.sub(r"\bPRICE\b", "CLOSE", e, flags=re.IGNORECASE)
    # Common SMA variants → SMA(n)
    e = re.sub(r"SMA\s*\(\s*(\d+)\s*\)", r"SMA(\1)", e, flags=re.IGNORECASE)
    e = re.sub(r"SMA\s*-\s*(\d+)",        r"SMA(\1)", e, flags=re.IGNORECASE)
    e = re.sub(r"SMA\s+(\d+)",            r"SMA(\1)", e, flags=re.IGNORECASE)
    # Sentiment words → numeric thresholds
    e = map_sentiment_words(e)
    return e

def normalize_plan_dict(d: dict) -> dict:
    """Make the LLM JSON line up with your Plan schema before validation."""
    d = dict(d)  # shallow copy
    # universe → universe_list
    if "universe_list" not in d and "universe" in d:
        d["universe_list"] = d["universe"]
        del d["universe"]

    # normalize gates only if present in this LLM shape
    gates = d.get("gates")
    if isinstance(gates, dict):
        for key in ("all_of", "any_of"):
            if isinstance(gates.get(key), list):
                for item in gates[key]:
                    if isinstance(item, dict) and "expr" in item:
                        item["expr"] = normalize_gate_expr(item["expr"])
    return d
...
def naive_parse(text: str) -> Plan:
    ...
    try:
        ...
        # Parse to dict, normalize, then validate into Plan
        raw_dict = json.loads(cleaned_output)
        norm_dict = normalize_plan_dict(raw_dict)

        try:
            # pydantic v2
            plan = Plan.model_validate(norm_dict)
        except AttributeError:
            # pydantic v1 fallback
            plan = Plan.parse_obj(norm_dict)

        print(f"Plan : {plan}") 
    except json.JSONDecodeError as e:
        raise ValueError(f"LLM generated invalid JSON after cleaning: {e}. Raw: {llm_output_string}")
    except Exception as e:
        raise ValueError(f"LLM JSON did not match Plan schema: {e}. Cleaned: {cleaned_output}")

    # for gate_list in [plan.gates.all_of, plan.gates.any_of]:
    #     for gate_expr in gate_list:
    #         gate_expr.expr = normalize_gate_expr(gate_expr.expr)
    print(f"Plan 2: {plan}") 
    return plan
...
def plan(req: PlanRequest):
    ...
    try:
        ...
        #analysis = analyze_features(generated_plan.model_dump())
...
def get_headlines():
    try:
        cp = fetch_headlines(auth_token=auth_token)
