...
def plan_to_summary(plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    overlays = []
    if "gates" in plan:
        if plan["gates"].get("all_of"): overlays.append("strict gates")
        if plan["gates"].get("any_of"): overlays.append("flex gates")
...
# ===================== ADD BELOW your helpers (reuse encode/decode_cursor) =====================
...
def create_strategy(
    ...
    # For true idempotency, add a dedicated table keyed by Idempotency-Key.
    try:
...
def _list_public(limit: int, cursor: Optional[str]) -> Page:
    ...
    if cursor:
        ...
        # PostgREST lacks tuple compare; do it with OR
        q = q.or_(f"and(created_at.lt.{created_at_iso}),and(created_at.eq.{created_at_iso},id.lt.{row_id})")
